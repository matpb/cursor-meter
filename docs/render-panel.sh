...
.bars{display:flex;flex-direction:column;gap:5px;width:170px}
...
<div class="row"><div class="lbl">Auto</div><div class="track"><div class="fill" style="width:${auto}%;background:${ac}"></div><div class="tick" style="left:${auto_tp}%"></div></div><div class="pct" style="color:${ac}">${auto}%</div></div>
<div class="row"><div class="lbl">API</div><div class="track"><div class="fill" style="width:${api}%;background:${pc}"></div><div class="tick" style="left:${api_tp}%"></div></div><div class="pct" style="color:${pc}">${api}%</div></div>
</div></div></body></html>
EOF
google-chrome-stable --headless --disable-gpu --window-size=320,90 --screenshot="$OUT" "file://$html" 2>/dev/null
rm -f "$html"
